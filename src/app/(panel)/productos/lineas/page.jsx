...
import { getLineas, createLinea, updateLinea, deleteLinea } from "@/services/productosService";
...
export default function ProductosLineasPage() {
    ...
    const load = async () => {
        try {
            ...
            setError(err.message || "No se pudo cargar lineas");
        } finally {
            setLoading(false);
        }
    };
    ...
    const onSave = async (payload) => {
        if (formModal.mode === "create") await createLinea(payload);
        else await updateLinea(formModal.item.id_linea, payload);
        setFormModal({ open: false, mode: "create", item: null });
        await load();
    };

    const onDelete = async () => {
        await deleteLinea(deleteItem.id_linea);
        setDeleteItem(null);
        await load();
    };
    ...
    return (
        <div className="space-y-4">
            <PageTitle title="Lineas" subtitle="Catalogo de lineas para productos" />
            ...
            <div className="bg-white rounded-2xl shadow-card border border-border overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {filtered.length === 0 && (
                            <tr>
                                <td colSpan={4} className="p-6 text-center text-muted">No hay lineas para mostrar.</td>
                            </tr>
                        )}
                        ...
                        {filtered.map((r) => {
                            ...
                            return (
                                <tr key={r.id_linea} className="border-t border-border hover:bg-background/50">
                ...
                <div className="fixed inset-0 z-50 bg-primary/40 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl border border-border shadow-card w-full max-w-md p-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-primary">Eliminar linea</h3>
...
function LineaFormModalInline({ mode, item, onClose, onSave }) {
    ...
    return (
        <form onSubmit={submit} className="bg-white rounded-2xl border border-border shadow-card w-full max-w-xl">
            <div className="flex items-center justify-between p-4 border-b border-border">
                <h3 className="text-lg font-semibold text-primary">{mode === "create" ? "Agregar linea" : "Editar linea"}</h3>
