...
export default function ComprasPage() {
    ...
    const load = useCallback(async () => {
        try {
            setLoading(true);
            setRows(await getCompras(search));
            setError("");
        } catch (e) {
    ...
    return (
        <div className="space-y-4">
            <PageTitle title="Compras" subtitle="Consulta de compras registradas" />

            {error ? <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">{error}</div> : null}

            <div className="flex gap-2 items-end">
                <Input label="Buscar" placeholder="Proveedor, usuario o estado" value={search} onChange={(e) => setSearch(e.target.value)} />
                ...
                <table className="w-full text-sm min-w-[900px]">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan={7} className="p-6 text-center text-muted">Sin compras</td></tr>
                        ) : rows.map((r) => (
                            <tr key={r.id_compra} className="border-t border-border hover:bg-background/50">
                                <td className="p-3">{r.id_compra}</td>
